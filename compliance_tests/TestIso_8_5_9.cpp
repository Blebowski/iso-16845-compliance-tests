        ...
        int Run()
        {
            ...
            for (size_t test_variant = 0; test_variant < test_variants.size(); test_variant++)
            {
                ...
                for (auto elem_test : elem_tests[test_variant])
                {
                     ...
                     *   3. Insert Passive Error frame from next bit to monitored frame.
                     ...
                     *         suspend field!
                     *   5. Append next frame. On monitor as is received by IUT, on driver as if
                     *      sent by LT. This checks that IUT becomes receiver of this frame!
                     *   6. Append the same frame as first frame once again (after this second
                     *      frame). This checks that DUT re-transmitts the first frame which had
                     *      error on 7-th data bit. Frame is as if received on driven frame.
                     *   7. Append one more intermission, since DUT will succesfully retransmitt
                     *      the frame and therefore go to suspend! This is needed to separate it
                     *      from next time step!
                     *****************************************************************************/
