        ...
        int Run()
        {
            ...
            for (size_t test_variant = 0; test_variant < test_variants.size(); test_variant++)
            {
                ...
                for (auto elem_test : elem_tests[test_variant])
                {
                     ...
                     *   3. Shorten third bit of Intermission by Phase 2 length and shorten Phase
                     *      1 segment by 1 TQ + 1 minimal time quanta (1 cycle). Do this in both
                     *      driven and monitored frames.
                     *   4. Turn Second driven frame received, but keep SOF dominant.
                     *   5. Turn SOF of second monitored frame to recessive (since it will not
                     *      be transmitted). Shorten that SOF by 1 TQ (since Hard sync ends with
                     *      one SYNC seg completed).
                     *   6. Append second frame after the first frame.
                     *   
                     *   Note: First frame ends 1 TQ - 1 minimal TQ before the end of Intermission
                     *         bit 2 (in both driven and monitored frames). After this, monitored
                     *         frame is appended, just with SOF dominant. This means that dominant
                     *         bit will be received by IUT as required! IUT will h-sync, but it
                     *         is still in intermission. So first bit of second frame will be tran-
                     *         smitted recessive. This is emulated by SOF of second monitored
                     *         frame!
                     *****************************************************************************/
