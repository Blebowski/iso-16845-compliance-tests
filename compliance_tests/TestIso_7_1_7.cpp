        ...
        int Run()
        {
            ...
            for (size_t test_variant = 0; test_variant < test_variants.size(); test_variant++)
            {
                ...
                for (auto elem_test : elem_tests[test_variant])
                {
                    ...
                    /**********************************************************************************
                     * Modify test frames:
                     *   1. Flip bit which shall cause protocol exception to dominant! This is needed
                     *      in CAN FD Enabled variant only, since in CAN FD Tolerant variant FDF bit
                     *      is sufficient to invoke protocol exception!
                     *   2. Update frame since CRC might have changed. This should not have impact
                     *      since IUT is going to protocol exception!
                     *   3. Turn monitored frame as if received. Force ACK to recessive since IUT is
                     *      in protocol exception!
                     *   4. Modify end of intermission as per elementary test. Add one bit of idle
                     *      in first elementary test case, remove one bit in last test.
                     *   5. Append second frame send by IUT. In third elementary test, IUT shall not
                     *      ack this frame. Force monitored ACK to recessive.
                     ...
                     * 
                     * Also CAN FD frames in model have two ACK bits therefore to get the sequence
                     * right (only 10 consecutive recessive in third elementary test), also second
                     * ACK bit must be forced to dominant!
                     * 
