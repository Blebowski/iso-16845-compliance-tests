        ...
        int Run()
        {
            ...
            for (size_t test_variant = 0; test_variant < test_variants.size(); test_variant++)
            {
                ...
                for (auto elem_test : elem_tests[test_variant])
                {
                    ...
                    /* We match bit position to be flipped with test index. Last two tests
                     * are tests where we test on SRR or IDE */
                    ...
                    /* On elementary test 31, LT send base frame. Correct the ID so that LT
                     * sends base ID with the same bits as IUT. This will guarantee that
                     * IUT sending extended frame with 0x00400000 will send the same first
                     * bits as LT. Since LT will be sending base frame, but IUT extended
                     * frame, IUT will loose on IDE bit!
                     */
                    else if (elem_test.index == 31)
                        id_lt = (id_iut >> 18) & 0x7FF;

                    /* On elementary test 31, IUT shall loose on IDE bit. This occurs when
                     * LT sends base frame! */
                    ...
                    /* On elementary test 31, IUT shall loose on IDE bit. After Base ID
                     * IUT will send recessive SRR. LT must also send recessive bit (RTR)
                     * otherwise IUT would loose on SRR and not IDE!
                     */
                    if (elem_test.index == 31)
                     ...
                     *      requested to send frame with this bit recessive (golden_frm_2).
                     *      Therefore this bit shall be expected recessive.
                     *      Bit position is calculated from elementary test index. First 11 tests
                     *      are in base identifier, next 18 are in Identifier extension.
                    ...
                    /* On elementary test 30, IUT shall loose on SRR bit, therefore we must send
                     * this bit dominant by LT, so we flip it
                     */
                    if (elem_test.index == 30){
                        ...
                        /* Forcing SRR low will cause 5 consecutive dominant bits at the end
                         * of base ID, therefore IUT inserts recessive stuff bit. Model does
                         * not account for this, so we must insert one extra bit in monitored
                         * frame. For driven frame, we must recalculate CRC!
                         */
                        driver_bit_frm->UpdateFrame();
                        monitor_bit_frm->InsertBit(BitType::Srr, BitValue::Recessive, index + 1);
                    }

                    /* On elementary test 31, IUT will be sending extended frame with the same
                     * base ID as LT. LT will be sending Base frame. But monitored frame is
                     * constructed from LTs frame which always has RTR bit dominant (right
                     * after Base ID). IUT is sending Extended frame, therefore at position of
                     * RTR it will send SRR which is Recessive. So bit at position of RTR in
                     * monitored frame must be set to recessive!
                     * 
                     * Note that in CAN FD frame there is no RTR bit so R1 must be set instead!
                     */
                    if (elem_test.index == 31){
                        if (test_variants[test_variant] == TestVariant::Common)
                            monitor_bit_frm->GetBitOf(0, BitType::Rtr)->bit_value_ =
                                BitValue::Recessive;
                        else
                            monitor_bit_frm->GetBitOf(0, BitType::R1)->bit_value_ =
                                BitValue::Recessive;
                    }
