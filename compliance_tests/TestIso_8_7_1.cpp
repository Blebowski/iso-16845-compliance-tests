        ...
        int Run()
        {
            ...
            for (size_t test_variant = 0; test_variant < test_variants.size(); test_variant++)
            {
                ...
                for (auto elem_test : elem_tests[test_variant])
                {
                     ...
                     *   2. Force 2-nd bit of data field (dominant preceeded by recessive),
                     *      to recessive from one time quanta after sample point to Recessive on
                     *      driven frame.
                     *   3. In second frame, force 2-nd bit of data field (same as before) to
                     *      recessive from one time quantum before sample point till the end of
                     *      frame to recessive. Also force last cycle of previous time quanta.
                     *      This accounts for "minimal time quanta" subtraction!
                     *   4. Insert Expected Error frame from next bit of data field. Insert to
                     *      both driven and monitored frame since TX/RX feebdack is disabled!
                     *   5. Append second frame to first frame.
                     *   6. Create next frame which is the same as first frame, but no values are
                     *      forced. Put ACK low.
                     *   7. Append frame from point 6 to test frame. This frame represents IUTs
                     *      retransmission due to error detected in previous frame.
                     *****************************************************************************/
                    ...
                    int cycles_length = bit_to_corrupt->GetTimeQuanta(start_index - 1)
                                            ->getLengthCycles();
                    bit_to_corrupt->GetTimeQuanta(start_index - 1)
                        ->ForceCycleValue(cycles_length - 1, BitValue::Recessive);
