        ...
        int Run()
        {
            ...
            for (int test_variant = 0; test_variant < test_variants.size(); test_variant++)
            {
                ...
                for (auto elem_test : elem_tests[test_variant])
                {
                     ...
                     *   2. Insert 16 dominant bits from next bit of monitored frame.
                     *      Insert 16 recessive bits from next bit of driven frame.
                     *      This emulates DUT always re-starting error detecting bit error during
                     *      active error flag.
                     *   3. Insert 6 recessive bits to emulate passive error flag (both driven and
                     *      monitored frames).
                     *   4. Insert 112 dominant bits to driven frame and 112 recessive bits to
                     *      monitored frame. Then Insert 8 + 3 + 8 (Error delimiter + intermission +
                     *      Suspend) to recessive frames.
                     *   5. Insert second frame as if transmitted by DUT. Append the same frame on
                     *      driven frame since TX/RX feedback is disabled! This is the same frame
                     *      as before because it is retransmitted by DUT!
                     *   6. Force first bit of intermission to dominant state -> Overload condition.
                     *      This is in fact 4th intermission bit (overall since there were) three
                     *      bits before!
                     *   7. Insert one dominant bit on monitored frame and one recessive bit on
                     *      driven frame. This emulates expected first bit of overload flag and
                     *      corruption of its first bit.
                     *   8. Insert 6+7 recessive bits on both driven and monitored frames. This 
                     *      emulates Passive error flag and error delimiter.
                     *   9. Insert one dominant bit to driven frame, and one recessive bit to
                     *      monitored frame. This represents next overload condition.
                     *  10. Insert 1 recessive bit to driven frame (error on first bit of
                     *      overload frame). Insert 1 dominant bit to monitored frame (first bit)
                     *      of overload frame. This should cause DUT to go Bus-off.
                     *  11. Insert 6 + 8 + 3 + 8 recessive bits to both driven and monitored frames.
                     *      This corresponds to Passive Error flag, Error delimiter + Intermission
                     *      + possible suspend.
                     *  12. Insert third frame as if sent by LT. In driven frame, this frame is as
                     *      if transmitted. In monitored frame, it is all recessive (including ACK)
                     *      since IUT shall be bus-off.
                     *****************************************************************************/
                    ...
                    /* Compensate ESI of second frame in secondelementary test. Then IUT is already
                     * passive!
                     */
                    if (test_variant == 1)
                    {
                    ...
                    driver_bit_frm->GetBitOf(3, BitType::Intermission)->bit_value_ =
                        BitValue::Dominant;

                    driver_bit_frm->GetBitOf(4, BitType::Intermission)->bit_value_ =
                        BitValue::Recessive;
                    monitor_bit_frm->GetBitOf(4, BitType::Intermission)->bit_value_ =
                        BitValue::Dominant;
